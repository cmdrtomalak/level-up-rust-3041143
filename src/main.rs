use std::{collections::{HashMap, HashSet, BinaryHeap}, cmp::Ordering, fmt::Binary};
...
fn shortest_path(g: &Graph, start: Node, goal: Node) -> Option<(Vec<Node>, Cost)> {
    // store the lowest cost distance to all nodes, start at infinity if we have not visited any nodes
    let mut dist: HashMap<Node, Cost> = g.nodes
        .iter()
        .map(|&x| if x == start { (x, 0) } else { (x, usize::MAX) })
        .collect();

    let mut pq = BinaryHeap::new(); // create priority queue of next nodes to visit

    pq.push(Step::from_start(start)); // Only know to visit start at this point

    while let Some(Step {cost, position, mut history}) = pq.pop() {
        if position == goal {
            history.push(goal);
        ...
        if let Some(destinations) = g.edges.get(&position) {
            for &(next_destination, next_cost) in destinations.iter() {
                ...
                // if path to this destination is lower than previous known path, add to visit
